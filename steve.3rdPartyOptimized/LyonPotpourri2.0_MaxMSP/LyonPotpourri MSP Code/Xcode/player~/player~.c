...
//  int active_events = x->active_events;
  ...
  /*
  for(i = 0; i < overlap_max; i++){
    if(events[i].increment == 0.0 && events[i].status == ACTIVE){
      post("killing a zombie at slot %d",i);
      events[i].status = INACTIVE;
    }
  }   */
  ...
  // if no to both, clean output vector and we're done
  if(bail){
    memset((void *)outchan,0,sizeof(float) * n);
    return(w+6);
  }
  ...
  for(i = 0; i < overlap_max; i++){
    if( events[i].status == ACTIVE){
	  ...
	  for(j = 0; j < n; j++){
		...
		// allow possibility of 0 increment (might happen if increment is live)
		if(increment >= 0){ // watch it
		  if(iphase == flimit ){
		    outchan[j] += b_samples[iphase] * gain;
		  } else {
    ...
    if(trigger_vec[i]){
	  ...
	  for(k=i; k<n; k++){
		...
		/*
		  #if MSP
		  if(! x->wavebuf->b_valid) {
		  post("%s: internal waveform replacement",OBJECT_NAME);
		  player_stop(x);
		  return(w+6);		
		  }
		  #endif
		*/
		...
		if(increment >= 0){ // note allowing zero increment
		  if(iphase == flimit ){
		    outchan[k] += b_samples[iphase] * gain;
		  } 
		  else {
...
//    post("starting pd player");
    dsp_add(pd_player, 5, x, 
	    sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[0]->s_n);	
  }
#endif
