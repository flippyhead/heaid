...
/* 	
  if( x->manual_override ){
    ...
    sum = x->asamps+x->dsamps+x->ssamps+x->rsamps;
    if( sum > x->tsamps) {
      fac = (float) x->tsamps / (float) sum;
      x->asamps = (float) x->asamps * fac;
      x->dsamps = (float) x->dsamps * fac;
      x->ssamps = (float) x->ssamps * fac;
      x->rsamps = (float) x->rsamps * fac;
      x->ebreak1 = x->asamps;
      x->ebreak2 = x->asamps+x->dsamps;
      x->ebreak3 = x->asamps+x->dsamps+x->ssamps;
    } else if (sum < x->tsamps ) {
      x->rsamps += x->tsamps - sum;
    }
  } else 
 */
