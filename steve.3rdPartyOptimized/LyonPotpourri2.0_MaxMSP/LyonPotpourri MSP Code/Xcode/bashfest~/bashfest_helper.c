...
void butterLopass( float *in, float cutoff, int frames, int channels, float srate)

{
  ...
  for( channel_to_compute = 0; channel_to_compute < channels; channel_to_compute++) {
    ...
    butter_filter( in, in, data, frames, channels, channel_to_compute);
  }

}

void butterBandpass(float *in, float center, float bandwidth, int frames,int  channels, float srate)
{
  int channel_to_compute;
  float data[8];
...
void butterHipass(float *in, float cutoff, int frames,int channels, float srate)
{
  int channel_to_compute;
  float data[8];
...
void feed1(float *inbuf, int in_frames, int out_frames,int channels, float *functab1,
	   ...
	   float duration, float maxDelay, t_bashfest *x)
{
  ...
  for(i = 0; i < out_frames*channels; i += channels ){
    ...
    inbuf[i] += delsamp2a = dliget2(delayLine2a, delay2, dv2a, srate);
    ...
    if( channels == 2 ){
      ...
      inbuf[i+1] += delsamp2b = dliget2(delayLine2b, delay2, dv2b, srate);
    }
  }
}
