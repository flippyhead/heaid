...
void bashfest_mute(t_bashfest *x, long t);
void bashfest_maximum_process(t_bashfest *x, long n);
void bashfest_minimum_process(t_bashfest *x, long n);
...
void bashfest_block_dsp(t_bashfest *x, long t);
...
void main(void)
{
  ...
  addmess((method)bashfest_soloproc,"soloproc", A_LONG, 0);
  addmess((method)bashfest_killproc,"killproc", A_LONG, 0);
  addmess((method)bashfest_latency,"latency", A_LONG, 0);
  addmess((method)bashfest_mute,"mute", A_LONG, 0);
  ...
  addmess((method)bashfest_maximum_process,"maximum_process", A_LONG, 0);
  addmess((method)bashfest_minimum_process,"minimum_process", A_LONG, 0);
  addmess((method)bashfest_block_dsp,"block_dsp", A_LONG, 0);
		...
		if(b_nchans == 1){ /* mono */
			...
			for(j = 0; j < n; j++){
			  if(events[i].countdown > 0){
			    ...
			    if( events[i].phasef < 0.0 || events[i].phasef >= x->wavebuf->b_frames){
	  ...
	  if(trigger_vec[i]){
		...
		for(j=0; j<overlap_max; j++){
		  if(events[j].status == INACTIVE){
		    ...
		    if(increment > 0){
		      ...
		      events[j].phasef = x->wavebuf->b_frames - 1;
		    }
		    insert_success = 1;
		    new_insert = j;
		    break;
		...
		if(!insert_success){ // steal a note
		  ...
		  if(increment > 0){
		    ...
		    events[new_insert].phasef = x->wavebuf->b_frames - 1;
		  }
		  insert_success = 1;
		...
		for(k=i; k<n; k++){
		  ...
		  if(events[new_insert].countdown > 0){
			...
			if(b_nchans == 1){
				...
				if(iphase < 0 || iphase >= x->wavebuf->b_frames){
			    ...
			    if( events[new_insert].phasef < 0.0 || events[new_insert].phasef >= x->wavebuf->b_frames){
